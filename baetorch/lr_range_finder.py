...
def run_auto_lr_range_v2(train_loader, bae_model, mode="mu", sigma_train="separate",
                      min_lr_range=0.0000001, max_lr_range=10,
                      reset_params=False, plot=True, verbose=True, save_mecha="copy", run_full=False, savefile="", savefolder="plots",
                         supervised=False, window_size=10, num_epochs=10):
    #helper function
    def round_sig(x, sig=2):
        return round(x, sig-int(floor(log10(abs(x))))-1)
    ...
    try:
        for epoch, (batch_idx, (data, target)) in product(range(num_epochs), enumerate(train_loader)):
