...
def calc_mean_results(predict_results, exclude_keys=[]):
    ...
    filtered_key_ids = [key_id for key_id,key in enumerate(keys) if key in filtered_keys]
    # filtered_values = np.array([values[key_id] for key_id in filtered_key_ids])
    filtered_values = {key:val for key,val in predict_results.items() if key in filtered_keys}
    # values = np.array([value.mean(1) for value in list(predict_results.values())])
    filtered_values = np.array([value.mean(1) for value in list(filtered_values.values())])
...
def calc_variance_dataset(*args, legends=["TEST", "OOD"], exclude_keys=[], savefile=""):
    ...
    for dataset_id in range(num_datasets):
        metrics_mean, axis_titles = calc_mean_results(args[dataset_id],exclude_keys=exclude_keys)
        means.append({"dataset":legends[dataset_id], "values":np.mean(metrics_mean,1)})
        variance.append({"dataset":legends[dataset_id], "values":np.var(metrics_mean,1)})
...
def save_csv_metrics(train_set_name,model_name, auroc_metrics, auprc_metrics, fpr80_metrics, metric_names,savefolder="results"):
    create_dir(savefolder)
    model_results = {"MODEL":model_name}
    auroc_dict = {metric_name+"_AUROC": auroc_metrics[metric_id] for metric_id, metric_name in enumerate(metric_names)}
    auprc_dict = {metric_name+"_AUPRC": auprc_metrics[metric_id] for metric_id, metric_name in enumerate(metric_names)}
    fpr80_dict = {metric_name+"_FPR80": fpr80_metrics[metric_id] for metric_id, metric_name in enumerate(metric_names)}
...
def save_csv_distribution(train_set_name, model_name, mean_dict, variance_dict, metric_names, savefolder="results"):
    create_dir(savefolder)
    model_results = {"MODEL":model_name}
    mean_dict = {mean_dict[dataset_id]["dataset"]+"_"+metric_name+"_MEAN": mean_dict[dataset_id]['values'][metric_id] for metric_id, metric_name in enumerate(metric_names) for dataset_id in range(len(mean_dict))}
    variance_dict = {variance_dict[dataset_id]["dataset"] + "_" + metric_name + "_VARIANCE": variance_dict[dataset_id]['values'][metric_id] for metric_id, metric_name in enumerate(metric_names) for dataset_id in range(len(variance_dict))}
