...
mat_file = mat_file_list[-3]  # hard ones = 0, 2, 10, -3
mat = loadmat(os.path.join(base_folder, mat_file))
X = mat["X"]
y = mat["y"].ravel()

# X = X[:, [246, 171, 111, 242, 161, 185, 173, 107, 273, 191]]
...
# time_method(lin_autoencoder.fit, temp_dataloader, num_epochs=200)
...
lin_autoencoder.fit(temp_dataloader, burn_epoch=150, sghmc_epoch=200, save_every=10)
# lin_autoencoder.fit(temp_dataloader, burn_epoch=10, sghmc_epoch=1500, save_every=100)
# lin_autoencoder.fit(temp_dataloader, burn_epoch=150, sghmc_epoch=0)
...
# auroc_features = []
# for feature_i in range(x_inliers_test.shape[-1]):
#     auroc_ood = calc_auroc(
#         ae_inliers_pred[nll_key][:, feature_i], ae_outliers_pred[nll_key][:, feature_i]
#     )
#     auroc_features.append(auroc_ood)
# print(np.sort(auroc_features))
# print(np.argsort(auroc_features)[-10:][::-1])
...
# =========================================
# concat_res = np.concatenate((ae_inliers_pred[nll_key], ae_outliers_pred[nll_key]))
# # X = X[:, [246, 171, 111, 242, 161, 185, 173, 107, 273, 191]]
#
# plt.figure()
# plt.hist(concat_res[:, [242]], alpha=0.5)
#
# plt.figure()
# plt.hist(ae_inliers_pred[nll_key][:, 242], alpha=0.5)
# plt.hist(ae_outliers_pred[nll_key][:, 242], alpha=0.5)
