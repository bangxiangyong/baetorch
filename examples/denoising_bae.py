...
train_loader = torch.utils.data.DataLoader(
    datasets.FashionMNIST('data-fashion-mnist', train=True, download=True,
                          transform=transforms.Compose([transforms.ToTensor()])), batch_size=train_batch_size, shuffle=True)
...
bae_mcdropout = BAE_MCDropout(autoencoder=autoencoder, dropout_p=0.2,
                              num_train_samples=5, num_samples=50, use_cuda=use_cuda, denoising_factor=noise_factor)
...
run_test_model(bae_models=bae_models, id_data_test=test_loader, ood_data_names=["OOD","NOISY"], ood_data_list=ood_data_list, id_data_name=train_set_name, output_reshape_size=(28, 28))
