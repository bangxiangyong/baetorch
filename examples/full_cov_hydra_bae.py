...
num_epoch = 500
num_clusters= 3
encoder = Encoder([DenseLayers(architecture=[20,100],
                               output_size=latent_dim,
                               input_size=x_train.shape[-1],
                               activation='leakyrelu',
                               last_activation='none')])
decoder_mu = infer_decoder(encoder, last_activation='none')
decoder_sig = CholLayer(architecture=[20,100], input_size=latent_dim, output_size=x_train.shape[-1])
autoencoder = Hydra_Autoencoder(encoder=encoder, decoder_mu=decoder_mu, decoder_sig=decoder_sig)
...
#======training====
bae_hydra_full_cov = BAE_Hydra(autoencoder=autoencoder, cluster_weight=gamma,
                               num_samples=num_samples, learning_rate=learning_rate,
                               likelihood="full_gaussian")

run_auto_lr_range_v2(convert_dataloader(x_train), bae_hydra_full_cov)
bae_hydra_full_cov.fit(x_train, num_epochs=pretrain_epoch, mode="mu")
bae_hydra_full_cov.fit(x_train, num_epochs=num_epoch, mode="sigma")


grid, grid_2d=get_grid2d_latent(x_train)
nll_full = bae_hydra_full_cov.predict_samples(grid_2d, select_keys=["nll_sigma"])
y_sig = bae_hydra_full_cov.predict_samples(grid_2d, select_keys=["y_sigma"])
se = bae_hydra_full_cov.predict_samples(grid_2d, select_keys=["se"])
