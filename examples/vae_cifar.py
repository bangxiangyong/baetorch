...
conv_architecture=[input_channel,24,48,64]

#specify encoder
#with convolutional layers and hidden dense layer
encoder = Encoder([ConvLayers(input_dim=input_dim,conv_architecture=conv_architecture, conv_kernel=[4,4,4],
                              conv_stride=[2,1,1], last_activation="sigmoid"),
...
bae_model = VAE(autoencoder=autoencoder,
                ...
                use_cuda=use_cuda,
                weight_decay=0.001, likelihood="bernoulli")
...
run_test_model(bae_models=bae_models, id_data_test=test_loader, ood_data_list=ood_data_list, id_data_name=train_set_name, output_reshape_size=(32, 32,3))
...
for ax_id,ax in enumerate(axes):
    if ax_id < num_draw_samples:
        ax.imshow(np.moveaxis(draw_decoded[ax_id].reshape(3,32,32),0,-1))
